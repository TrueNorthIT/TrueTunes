...
export function AlbumPanel({ item, onAddToQueue, onOpenArtist }: Props) {
  ...
  const totalSecs =
    data?.tracks.reduce((s, t) => s + t.durationSeconds, 0) ?? 0;
      ...
      >
        ...
        {data?.tracks.map((track, i) => (
          <div
            ...
            className={[styles.trackRow, selected.has(i) ? styles.selected : ""]
              .filter(Boolean)
              .join(" ")}
            draggable
            ...
            onDragStart={(e) => {
              ...
              ghost.textContent =
                toMove.length > 1 ? `${toMove.length} tracks` : track.title;
